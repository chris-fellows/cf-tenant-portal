...
namespace CFTenantPortal.Controllers
{
    ...
    */
    public class HomeController : Controller
    {
        ...
        public IActionResult SendMessage(string? issueId, string? messageTypeId, string? propertyId, string? propertyOwnerId)
        {
            ...
            // Check parameters set. If they indicate a specific entity then don't allow user to select other entity
            if (!String.IsNullOrEmpty(issueId))
            {
                issues = issues.Where(i => i.Id == issueId);

                // If issue relates to single property then ensure that user can't select other property
                if (!String.IsNullOrEmpty(issues.First().PropertyId) && String.IsNullOrEmpty(propertyId))
                {
                    propertyId = issues.First().PropertyId;
                }

                // If issue relates to single property group then ensure that user can only select one of those properties
                if (!String.IsNullOrEmpty(issues.First().PropertyGroupId) && 
                    String.IsNullOrEmpty(issues.First().PropertyId))                    
                {
                    properties = properties.Where(p => p.GroupId == issues.First().PropertyGroupId);
                }
            }
            if (!String.IsNullOrEmpty(propertyId))   // Only display specific property
            {
                properties = properties.Where(p => p.Id == propertyId);

                // Only display property owner for this property
                propertyOwnerId = properties.First().OwnerId;
            }            
            if (!String.IsNullOrEmpty(propertyOwnerId)) // Only display specific property owner
            {
                propertyOwners = propertyOwners.Where(po => po.Id == propertyOwnerId);

                // Only display properties for this owner
                properties = properties.Where(p => p.OwnerId == propertyOwnerId);
            }
            ...
            var model = new SendMessageVM()
            {
                ...
                IssueId = String.IsNullOrEmpty(issueId) ? EntityReference.None.Id : issueId,
                // If specific issue set then display only that item in dropdown
                IssueRefList = issues.Select(i =>
                                        {
                                            return _mapper.Map<EntityReference>(i);
                                        }).ToList(),                           
                MessageTypeId = String.IsNullOrEmpty(messageTypeId) ? messageTypes.First().Id : messageTypeId,
                // If specific message type set then display only that item in dropdown
                MessageTypeRefList = messageTypes.Select(mt =>
                                        {
                                            return _mapper.Map<EntityReference>(mt);
                                        }).ToList(),                                
                PropertyId = String.IsNullOrEmpty(propertyId) ? EntityReference.None.Id : propertyId,
                // If specific property set then display only that item in dropdown
                PropertyRefList = properties.Select(p =>
                                        {
                                            return _mapper.Map<EntityReference>(p);
                                        }).ToList(),                                
                PropertyOwnerId = String.IsNullOrEmpty(propertyOwnerId) ? propertyOwners.First().Id : propertyOwnerId,
                // If specific property owner set that item in dropdown
                PropertyOwnerRefList = propertyOwners.Select(po =>
                                        {
                                            return _mapper.Map<EntityReference>(po);
                                        }).ToList()                                   
            ...
            // Add None for optional properties for entities not fixed. Message type & property owner are mandatory
            if (String.IsNullOrEmpty(issueId)) model.IssueRefList.Insert(0, EntityReference.None);            
            if (String.IsNullOrEmpty(propertyId)) model.PropertyRefList.Insert(0, EntityReference.None);            

            return View(model);
        }

        /// <summary>
        /// Processes form to send message
        /// </summary>
        /// <param name="sendMessage"></param>
        /// <returns></returns>
        public IActionResult SendMessageForm(SendMessageVM sendMessage)
        {
            var xxx = 1000;
